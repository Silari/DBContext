...
async def updatepiczel() :
    if not client.is_closed(): #Keep running until client stops
        #Store the old list in a new var for comparison later
        oldlist = parsed
        #Update parsed with the current API results.
        if not await updateparsed() :
            #Updating failed for some reason - Bad Gateway seems common
